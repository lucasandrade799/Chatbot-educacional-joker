...
def handle_router():
    """
    Rota principal que recebe a mensagem, autentica e a envia ao Gemini para
    roteamento e execução da função correta.
    """
    try:
        data = request.get_json()
...
        
        if not mensagem or tipo_usuario not in ['ALUNO', 'PROFESSOR']:
            return jsonify({"error": "Mensagem ou Tipo de Usuário inválido/ausente."}), 400

        # Roteia a mensagem para o Gemini e obtém a resposta processada
        resposta_joker = rotear_e_executar_mensagem(mensagem, tipo_usuario)

        # A resposta do Joker já está formatada pelo Gemini e será enviada diretamente na chave 'message'
        return jsonify({"message": resposta_joker}), 200

    except Exception as e:
        print(f"❌ Erro na rota /router: {e}")
        return jsonify({"error": "Erro interno do servidor ao processar a requisição."}), 500


@app.route('/')
def serve_index():
    """Serva o arquivo joker_bot.html principal, que está na raiz."""
    # Presume que o arquivo HTML (joker_bot.html) está na mesma pasta
    return send_file('joker_bot.html') 

if __name__ == '__main__':
    init_db()
    # O modo debug permite recarregar o servidor automaticamente ao salvar o código.
